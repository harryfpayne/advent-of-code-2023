...
func main() {
	...
	t := time.Now()
	edges := MinCut(components)
	fmt.Println(edges)
	for _, e := range edges {
		delete(components[e[0]].Connections, e[1])
		delete(components[e[1]].Connections, e[0])
	}

	fmt.Println("Part 1:", Subgraph(components))
	fmt.Println(time.Since(t))
}

func Parse(str string) map[string]Component {
	...
	for _, comp := range strings.Split(str, "\n") {
		...
		for _, s := range parts[1:] {
			components[name].Connections[s] = struct{}{}
		}
	}

	for name, component := range components {
		for connected := range component.Connections {
			if _, ok := components[connected]; !ok {
				components[connected] = Component{
					Name:        connected,
					Connections: make(map[string]struct{}),
				}
			}
			components[connected].Connections[name] = struct{}{}
		}
	}
