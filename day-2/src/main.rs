...
impl Round {
    ...
    fn power_sum(&self) -> i32 {
        self.red * self.green * self.blue
    }
}

fn parse_game(game_string: &str) -> Game {
    let (game_string, rounds_string) = game_string.split_once(":")
        .expect("invalid game string, no ':'");

    let game_number = game_string.strip_prefix("Game ")
        .expect("invalid game string, no 'Game '")
        .parse::<i32>()
    ...
    for round_string in rounds_string.split(";") {
        ...
        for color_count in round_string.trim().split(", ") {
            let (count, color) = color_count.split_once(" ").expect("invalid color string");
            let count = count.parse::<i32>().expect("invalid color count");
...
fn part_one(puzzle: &str) -> i32 {
    let games = puzzle.trim().lines().map(parse_game);
    ...
    for game in games {
        ...
        'round_loop: for round in game.rounds {
...
fn part_two(puzzle: &str) -> i32 {
    ...
    let mut sum = 0;
    for game in games {
        let mut round_max = Round::empty();

        for round in game.rounds {
            if round.red > round_max.red {
                round_max.red = round.red;
            }
            if round.green > round_max.green {
                round_max.green = round.green;
            }
            if round.blue > round_max.blue {
                round_max.blue = round.blue;
            }
        }

        sum += round_max.power_sum();
    }
