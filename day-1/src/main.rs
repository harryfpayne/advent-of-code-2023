...
fn part_one(s: &str) -> i32 {
    ...
    for line in s.lines() {
        ...
        for c in line.chars() {
            if c.is_numeric() {
                numbers.push(c.to_string().parse::<i32>().unwrap())
            }
        }
        if let Some(f) = numbers.first() {
            count += f * 10;
        }
...
fn part_two(s: &str) -> i32 {
    ...
    let word_lengths = words.keys().fold(Vec::new(), |mut acc, key| {
        if !acc.contains(&key.len()) {
            acc.push(key.len())
        }
        acc
    });
    ...
    for line in PUZZLE.lines() {
        ...
        for idx in 0..line.len() {
            if let Some(c) = line.get(idx..idx+1) {
                let first_letter = c.chars().next().expect("got empty string");
                if first_letter.is_numeric() {
                    numbers.push(first_letter.to_string().parse::<i32>().unwrap())
                } else {
                    word_lengths.iter().for_each(|len| {
                        let upper_boundary = (idx+len).min(line.len());
                        let sub_word = line.get(idx..upper_boundary).expect("invalid string range");
                        if let Some(value) = words.get(sub_word) {
                            numbers.push(*value);
                        }
                    })
                }
...
fn main() {
    let count = part_two(PUZZLE);

    println!("{}", count)
}

const PUZZLE: &str = "nkzjrdqrmpztpqninetwofour1znnkd
